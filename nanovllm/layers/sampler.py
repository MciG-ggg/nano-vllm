...
class Sampler(nn.Module):
    ...
    Default path (``top_k=0``, ``repetition_penalty=1.0``) is identical to
    the pre-extension behaviour: temperature scaling + Gumbel-max.
    ...
    Behaviour matches top-k + multinomial up to float rounding; the
    Gumbel-max tail is bit-exact when ``top_k=0`` and
